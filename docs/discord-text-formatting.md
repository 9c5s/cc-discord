...
### コード装飾の中ではマークダウンの「表示」が無効化される

インラインコードとコードブロックの中では `*` `_` `~~` `||` などの装飾、メンション、リンク化は
表示上は無効化され、書いたまま表示される。本プラグインのツール通知は全体をコード装飾で囲んでいる
(`summarize.ts` の `code()`) ため、ツール引数に装飾文字が含まれても表示は安全である。
**ただしメンションの ping はコード装飾では防げない (次項)。**

### コード装飾はメンションの ping を防がない (実機確認済み)

`@everyone` / `<@USER_ID>` をインラインコードに入れて bot から送信したところ、表示は raw テキストの
ままだが**メンション通知として受信ボックスに入った** (2026-06-10 実機確認)。Discord API はメッセージ
content 全体から mention を解析するため、コード装飾の内側かどうかは ping の発生に関係しない。
メンション ping の確実な防御は `allowed_mentions` フィールドだけである。
