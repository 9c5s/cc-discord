...
`archiveStaleProgressThreads()` は bot 参加 guild の active threads を `fetchActiveThreads()` で取得し、以下の AND 条件で絞ったものだけ `setArchived(true)` する:
...
- **archive 対象判定の厳密さ**: bot 参加 guild 全体をスキャンするため、判定条件を緩めると他用途のスレッドを誤閉鎖しうる。4 条件 AND を維持する。
- **archive タイミング**: 単発で完結した会話は最大 60 分 + 5 分 = 65 分でアーカイブされる。Discord 標準の 60 分とほぼ同等で、運用上の体感差はない。
- **過去の滞留スレッド**: 起動直後の即時実行で routes 配下が対象になる。bot 参加 guild 全体のスキャンに変えれば routes 範囲外も同じ手順で掃除できる。
