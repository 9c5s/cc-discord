...
パッチ C で進捗用スレッドを `autoArchiveDuration: 60` で作っているが、Discord 側の auto-archive が API レベルで発火しないケースが実測された (`archived=false` のまま日数単位で滞留)。cc-discord 側で進捗の流れが止まる契機に `setArchived(true)` を明示発行する。

### 接続点

- 新規 inbound: owner-level write-lock を取得 → 次宛先を決定 (anchor + startThread の `await` もロック内で inbound 順を保つ) → `prev = readFileSync(ptFile)` + `writeFileSync(ptFile, newTarget)` を同期で実行 → ロック解放 → `prev !== chat_id && prev !== newTarget` なら `setTimeout(3s)` で遅延 archive。
- reply 経路では archive しない。MCP ツール呼び出し時点の ptFile と並走 inbound の上書き順序を確実に紐付ける手段が無く、安全側に倒して handleInbound 側に一本化した。

`archiveProgressThread()` は `isThread() + archived === false + ownerId === botUserId` の AND で対象を絞り、それ以外 (DM/親チャンネル/ユーザー作成スレッド) は no-op。失敗は全て無視する。

### 注意すべきポイント

- **archive は bot 作成スレッド限定**: owned channel 配下の既存ユーザースレッドから inbound が来ると `chat_id` がそのスレッド ID で ptFile に入る。`ownerId === botUserId` で弾かないと、bot に管理権限がある場合にユーザースレッドが誤って閉じられる。
- **write-lock は `await` の前に取る**: startThread 完了順にロックを取ると、早い inbound A が遅い startThread の間に 後の inbound B が先に ptFile を上書きし、A が再開時に B を `prev` として archive してしまう順序逆転が起きる。lock を anchor の前に取れば inbound 順で直列化される。critical section に startThread の API レイテンシ (100-500ms) を含むが、並行 inbound はロック待ちで詰まる。最大 3 秒待っても取れない場合はロック無しで続行する (滞留リスクは残るが処理は止めない)。
- **archive は ptFile 上書き「後」かつ 3 秒遅延で呼ぶ**: `plugin/src/notify.ts` は `progressChannelId()` を local 変数に capture してから fetch を `await` するため、上書き直前に旧 prev を capture した watch iteration が遅れて POST を完了させると Discord の自動 unarchive (archived スレッドへの POST で自動的に open される仕様) で前スレッドが再アクティブ化する。watch 周期 (250ms) を十分超える 3 秒の `setTimeout` で in-flight POST の完了を待つ。`tid.unref?.()` で Node.js プロセスの終了をブロックしない。
- **prev が `chat_id` または `newTarget` と同じならスキップ**: スレッド内連続 inbound では prev と次宛先が同じ ID になり、archive 後に同じ ID で再利用されると watcher が archived スレッドへ POST し続ける。
- **過去の滞留スレッドは API レベルで一括 `setArchived`**: 本パッチは新規分のみ救う。既存の滞留は運用者側で API レベルに `setArchived(true)` を一括投げる。判定は `owner_id == bot + 名前パターン + autoArchiveDuration == 60` の AND が安全で、bot 参加 guild の active threads 全体をスキャンすれば routes/* 範囲外でも対応できる。
