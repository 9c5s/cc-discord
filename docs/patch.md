...
- 最終メッセージ (snowflake から派生) または `createdTimestamp` から 12 時間以上経過

### 注意すべきポイント

- **active なスレッドには絶対に触らない**: 12 時間以上 inactive を条件にしているため、進行中の会話や watch の遅延 POST が来うる時間帯のスレッドは対象外。Discord 標準の 60 分閾値より大きく取ることで、active turn 中の長い shell/build (60 分以上 silent) も巻き込まないし、scanner の rmSync と handleInbound の write の時間軸がほぼ重ならず file race も現実発生しない。
...
- **archive タイミング**: 単発で完結した会話は最大 12 時間 + 5 分でアーカイブされる。Discord 標準の 60 分より長いが、Discord の auto-archive が日数単位で発火しない不発ケースの fallback としては十分。
