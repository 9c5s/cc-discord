...
担当名を持たないセッション (担当ディレクトリが未設定、または正規化名が空) では、この解決自体を行わない。
どのチャンネル名とも一致しないため、一覧の取得は rate limit の枠を使うだけになる。
...
3. 宛先の読み直し
4. activation の最終再確認
5. 再確認の直後に、待機を挟まず投稿する

`retry_after` を待った後の再送は新しい送信試行として扱い、1 からやり直す。
待機の間に activation や allowlist が変わった場合に、古い判定のまま投稿しないためである。

3 の読み直しは、gate の待ちの間に同じ activation の次の inbound が宛先を差し替えていないかを確かめる。
差し替えられていたら 1 からやり直す。
そのまま投稿すると、進捗が前のやり取りのスレッドへ出る。
...
proxy の終了処理で消すのは自分の heartbeat だけで、ポインタと宛先は残す。
MCP だけが再起動される経路では SessionStart が発火せず、誰もこれらを作り直さないためである。
消すと、走っている turn の進捗がその場で切れ、以後の inbound も宛先を持てないまま処理される。

残しても誤用は起きない。
heartbeat が止まれば watcher は待機へ移って 30 秒で終了し、`run_id` が一致しないポインタは現行 activation として採用しない。
残ったファイルは、ポインタなら次の SessionStart による置き換えか 7 日後の掃除で、宛先なら 12 時間の失効で消える。
...
「見えていない側に同名の候補があると、曖昧と判定できないまま 1 件目を担当にしてしまう」という指摘も受けたが、判断は変わらない。
201 個目の guild が現れる時点で前提が崩れており、そのときはページングだけでなく解決の設計ごと見直す。

### proxy のログは debug の設定で絞らない

複数セッションが同じ通知を受ける構成では、担当外のセッションが inbound ごとに破棄の記録を書く。
ローテーションも上限も無いため、通常の運用でログが際限なく増えるうえ、通知ごとに同期の書き込みが挟まる、という指摘があった。

増え方は実測で確認した。
2 日以上運用した時点で、7 つの担当を合わせてログは 26KB、93 行だった。
`inbound dropped` を含めても、この規模では上限もローテーションも要らない。

書き込みの負担より、記録が残ることの価値のほうが大きい。
複数セッションが同じ通知を受ける構成では、どのセッションがどの理由で破棄したかが切り分けの手がかりになる。
実際、inbound が届かない障害では、破棄の記録が無いことから「担当外として捨てられたのではない」と絞り込んでいる。
debug の設定の裏に隠すと、障害のたびに設定を変えて再現を待つことになる。

なお、指摘が示した「コメントと実装の食い違い」は正しかったので直した。
コメントは「詳細は `DISCORD_NOTIFY_DEBUG` のときだけ記録する」と書いていたが、実装は常時記録している。
コメントを実装に合わせた。

### 配送の直前に担当を確かめ直さない

スレッドの作成と activation の解決を待つ間に担当が変わると、配送の直前の判定は鮮度しか見ないため、変更前の担当のまま 1 件が通る、という指摘があった。

担当の変更は 60 秒ごとの解決でしか起きず、そこに inbound の処理が数秒の幅で重なる必要がある。
通ってしまった場合でも、宛先は変更前の担当チャンネル配下に作ったスレッドで、担当外のチャンネルや他のユーザーへは出ない。
その後の進捗の送信は毎回 outbound gate を通るため、2 件目以降は止まる。

同じ理由で、判定の回数を増やす修正は既に一度取り下げている。
発生条件が非現実的な経路のために判定を重ねると、分岐が増えて追いにくくなるだけである。
