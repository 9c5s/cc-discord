...
ただし据え置きは最後に解決しきれた時刻から 5 分までで、そこを超えたら担当を手放して配送も進捗も止める。
...
| `progress-thread/<owner>.lock-<message_id>` | inbound の排他ロック |
...
`progress-thread/<owner>.lock-<message_id>` を 60 秒で回収すると、処理に 60 秒以上かかったプロセスと、後からロックを取り直したプロセスが同じ inbound を二重に処理しうる、という指摘があった。

ロックの取得は通知を受け取った瞬間の 1 回だけで、取れなかったプロセスはその通知を破棄する。
回収後に同じ `message_id` をもう一度取得する経路が実装に存在しないため、この二重処理は起こらない。
回収は、取得したプロセスが異常終了した場合にファイルが残り続けるのを防ぐためだけにある。
