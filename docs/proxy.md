...
子から来たメッセージのうち、通知だけは中継のループから切り離して処理する。
通知の処理はチャンネル実体の取得、担当解決、スレッドの作成と REST を重ねるため、秒単位で待つことがある。
中継のループはハンドラの完了を待ってから次の行へ進むので、そのまま載せると、子が既に書いた `tools/call` や `initialize` の応答が通知の後ろで滞り、Claude Code 側がタイムアウトしうる。
通知どうしの順序はキューで保つため、ロックの取得と宛先の書き込みが前後することはない。
...
スレッドを立てるのは、現行 activation を解決できたときだけである。
`CC_DISCORD_RUN_ID` を持たないプロセスや、SessionStart hook がポインタを書けなかったセッションでは、宛先を書けず watcher も動かない。
そこでスレッドだけ作ると、誰も使わない空のスレッドが inbound のたびに残る。
...
- heartbeat が失効している間は transcript を読まない (読み取り位置だけ進めると、その分の進捗を取り戻せない)
- ポインタが別の activation を指していたら、送信キューを破棄して終了する
- heartbeat が失効しただけなら待機へ戻り、そこから 30 秒待って戻らなければ終了する
- 投稿の直前の確認で heartbeat が失効していた場合は、その 1 件を送らずに諦める (終了はしない)
...
1. ポインタの確認 (別の activation を指していたら watcher ごと終了する)
2. heartbeat の確認 (失効していたら送らずに諦める)
3. outbound gate (宛先の実体を取得し、guild は担当の決定関数と、DM は allowFrom と照合する)
4. 宛先の読み直し
5. 1 と 2 の再確認
6. 再確認の直後に、待機を挟まず投稿する

`retry_after` を待った後の再送は新しい送信試行として扱い、1 からやり直す。
待機の間に activation や allowlist が変わった場合に、古い判定のまま投稿しないためである。

2 で終了させないのは、tick と同じ理由である。
heartbeat の失効は proxy の書き直しが遅れているだけのことがあり、終了の判断は tick 側が待機を経て下す。
送信の側で終了を返すと、tick で待機へ戻す仕組みを迂回して watcher が終わってしまう。

4 の読み直しは、gate の待ちの間に同じ activation の次の inbound が宛先を差し替えていないかを確かめる。
差し替えられていたら 1 からやり直す。
...
宛先の置き換えに失敗したときは、前の宛先を消す。
置き換えは rename なので、読み手と競合すると失敗しうる。
前の宛先を残すと、新しいやり取りの進捗が前のスレッドへ出るため、進捗が欠けるほうを選ぶ。
...
`access.json` を書くのは `/cc-discord:access` と、`pairing` の `pending` を保存する公式 server の 2 者である (`/cc-discord:configure` は読むだけである)。
どちらも読み込みから書き戻しまでの間に相手の更新を取りこぼしうるため、共有ロック付きの更新境界へまとめるべき、という指摘があった。

access の管理はこの 2 者の領分で、proxy は読むだけである。
片方だけがロックを守っても競合は消えず、公式のコピーである skill に規約を足すと、上流が変わるたびに追随する対象が増える。
競合の窓は「ペアリングの承認と未知の DM の到着が重なる」ときに限られ、取りこぼされた側はもう一度ペアリングすれば復旧する。

### take over 中の要求のキャンセルを追わない

`reply` や `edit_message` の実行中に Claude がキャンセルすると、`notifications/cancelled` は子へ転送される。
子は take over した要求を知らないので何もせず、proxy は自分の REST を続けて残りのチャンクを投稿しうる、という指摘があった。

take over した要求の id を覚えて中断する仕組みは持たない。
`reply` が複数チャンクに分かれるのは 2000 文字を超える返信に限られ、その投稿中にキャンセルが届く幅は狭い。
加えて、届いたところで先に投稿したチャンクは消えないため、「何も出さない」という意図はどちらにせよ達成できない。
中断の機構を足すより、部分的な返信が残ることを受け入れる。

### 期限切れの宛先を消す前に読み直さない

`listTargets()` で列挙してから `deleteTarget()` を呼ぶまでの間に、別のプロセスが同じ `.meta` を新しい inbound で置き換えると、新しい宛先のほうを消してしまう、という指摘があった。

窓は列挙と削除の間のミリ秒で、しかもその瞬間に、ちょうど 12 時間を過ぎた宛先が同じ activation の新しい inbound で更新される必要がある。
消えたとしても、影響は次の inbound で作り直されるまで進捗が出ないことに限られる。

読み直して同一性を比べても、その比較と削除の間に同じ窓が残る。
競合を本当に消すには削除を条件付きにする仕組みが要るが、失われるものの大きさに見合わない。

### typing に ref-count を持たせない

同じチャンネルで inbound が重なると、先に来た方の返信で入力中の表示が消える。
旧実装のようにチャンネルごとの残数を持てば、後から来た方の処理中も表示を続けられる、という指摘があった。

残数方式には、返信されないターンで残数が減らないという欠点がある。
Claude Code の処理は中断できるため、`reply` が呼ばれないまま終わるターンは珍しくない。
その分の残数は誰も減らせず、安全弁の 10 分まで入力中の表示が残る。

どちらの方式にも表示が実態とずれる場面があり、ずれる向きが違うだけである。
「処理中なのに消える」より「処理していないのに出続ける」ほうが誤解を招くため、無条件に停止する側を選んでいる。
この判断は「typing の継続」にも書いてある。

### 子の close の後に中継の drain を待たない

子の `close` は OS のストリームが閉じたことを示すだけで、`readJsonLines` のキューに未処理の行が残っていることがある。
そこで終了すると、最後の JSON-RPC 応答を捨てうる、という指摘があった。

公式 0.0.4 が自分から終了する経路は 4 つあり、いずれもこの状況にならない。
トークンの欠如と login の失敗は、応答を書く前に終わる。
残る 2 つは stdin の EOF と SIGTERM で、どちらも proxy が終了を決めた後にしか起きない。
proxy が `child.stdin.end()` を呼ぶのは自分の終了処理の中であり、SIGTERM を送るのも proxy である。

つまり「子が最後の応答を書いてから自分の判断で終了する」経路が無い。
drain と close の両方を待つ形にすると、ハンドラが返らないときに終われなくなるため、そのためのタイムアウトも要る。
起こらない経路のために状態を増やさない。
...
- `CC_DISCORD_RUN_ID` を設定せずに起動したプロセス、hook に `CLAUDE_PID` が渡らない環境、MCP サーバーが Claude Code の直接の子として起動されない環境では、進捗の転送が無効になる。
  このとき進捗スレッドも作らないため、返信だけが親チャンネルへ届く。
