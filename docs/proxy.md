...
条件を緩めると、同じチャンネル内の他用途のスレッドを誤って閉じうるため、5 条件の AND を維持する。
...
スレッドを指したままの宛先を残して閉じると、watcher が投稿して開き直すためである。
...
| `progress-thread/<owner>.lock-<message_id>` | inbound の排他ロック。12 時間で回収する |
...
### archive の前に消した宛先は復元しない

滞留スレッドを閉じる前に宛先を消すため、archive の PATCH が失敗すると宛先だけが失われる、という指摘があった。

archive の対象は 12 時間動きの無いスレッドに限られる。
宛先はスレッドを作った時点で書かれるので、対象スレッドを指す宛先は必ず 12 時間以上前のものであり、宛先の有効期間 (12 時間) を過ぎて既に無効である。
したがって失われるのは無効な宛先だけで、進捗の転送は影響を受けない。

同じ理由で、宛先の読み取りが一時的に失敗して「宛先なし」と見えたまま archive しても、watcher が閉じたスレッドへ投稿することはない。
宛先の削除を確認してから閉じるのは、この前提が崩れたときのための保険である。
