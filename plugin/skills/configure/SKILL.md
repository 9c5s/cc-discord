...
3. **What next** — end with a concrete next step based on state:
   - No token → *"Run `/cc-discord:configure <token>` with your bot token from
     the Developer Portal → Bot → Reset Token."*
   - Token set, policy is pairing, nobody allowed → *"DM your bot on
     Discord. It replies with a code; approve with `/cc-discord:access pair
     <code>`."*
...
3. **If yes and policy is still `pairing`** → *"Good. Let's lock it down so
   ...
   `/cc-discord:access policy allowlist`. Do this proactively — don't wait to
   be asked.
4. **If no, people are missing** → *"Have them DM the bot; you'll approve
   each with `/cc-discord:access pair <code>`. Run this skill again once
   ...
   right-click them → Copy User ID, then `/cc-discord:access allow <id>`."*
