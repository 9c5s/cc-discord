...
test('progressChannelId: progress-thread ファイルが無ければ null を返し送信しない', () => {
  // inbound が来る前のローカル作業の進捗を channelId フォールバックで漏らさない
  process.env.CLAUDE_PROJECT_DIR = '/projects/cc-discord'
  writeRoute('cc-discord', '111111111')
  expect(progressChannelId()).toBeNull()
})

test('progressChannelId: progress-thread ファイルが空白のみでも null を返す', () => {
  process.env.CLAUDE_PROJECT_DIR = '/projects/cc-discord'
  writeRoute('cc-discord', '111111111')
  const threadDir = join(process.env.DISCORD_STATE_DIR!, 'progress-thread')
  mkdirSync(threadDir, { recursive: true })
...
async function withMockedFetch(
  ...
  // 送信先は progress-thread ファイルが唯一の源 (inbound 到来後の状態を再現する)
  const threadDir = join(process.env.DISCORD_STATE_DIR!, 'progress-thread')
  mkdirSync(threadDir, { recursive: true })
  writeFileSync(join(threadDir, 'cc-discord'), '123456789', { encoding: 'utf8' })
