...
import { toolSummary, thinkingGist, threadName, statusLine } from '../src/summarize'
...
test('statusLine はモデルとコンテキスト使用量を1行のコードブロックにする', () => {
  const usage = { input_tokens: 1, cache_read_input_tokens: 88000, cache_creation_input_tokens: 1141, output_tokens: 1931 }
  expect(statusLine('claude-fable-5', usage)).toBe('```\nclaude-fable-5 | ctx 89.1k | out 1.9k\n```')
})

test('statusLine は欠けた usage フィールドを 0 として扱う', () => {
  expect(statusLine('m', {})).toBe('```\nm | ctx 0.0k | out 0.0k\n```')
})
